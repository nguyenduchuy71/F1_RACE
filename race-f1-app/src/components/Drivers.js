...
function Drivers() {
    const [getListDriverResult, list_year, selected_year, setSelectedYear,
            list_winner_info, list_pts_info, list_background_color ] = useDriverStore((state) => [
    ...
    return (
        <div className="my-4 px-4 min-h-screen flex flex-col items-center">
            ...
            <BarChartElement list_year_info={list_winner_info} list_pos_info={list_pts_info} 
                list_background_color={list_background_color} axis={'x'} x_label={'Drivers'}
                y_label={'Points'} title={`The ranking of the drivers of the year ${selected_year}`} />
        </div>
    )
}
