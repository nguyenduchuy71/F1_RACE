...
function Race() {
    const [list_race_result, getListRaceResult, list_year, selected_year,
            setSelectedYear, list_winner, selected_winner, setSelectedWinner,
            list_car, selected_car, setSelectedCar, search_text, setSearchText] = useRaceStore((state) => [
    ...
    useEffect(() => {
        getListRaceResult();
    }, [getListRaceResult, setSelectedWinner, setSelectedYear, setSelectedCar, setSearchText]);
    ...
    <div className="my-4 px-4 min-h-screen flex justify-center flex-row items-stretch flex-wrap-reverse">
        <div className="overflow-hidden rounded-lg border border-gray-200 shadow-md mr-5 flex-grow">
            <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
                ...
                <tbody className="divide-y divide-gray-100 border-t border-gray-100 overflow-auto">
                    {list_race_result && list_race_result.map((value, index) => 
                        {
        ...
        <div className="max-h-min flex flex-col items-stretch">
            ...
            <div className="bg-white/60 flex flex-col p-1 rounded-lg mt-4">
                ...
                <SelectWinner listwinner={list_winner} selectedWinner={selected_winner} onSetSelectedWinner={setSelectedWinner} />
            </div>
            <div className="bg-white/60 flex flex-col p-1 rounded-lg mt-4">
                <p className="text-gray-700">Select a team:</p>
                <SelectCar listcar={list_car} selectedCar={selected_car} onSetSelectedCar={setSelectedCar} />
            </div>
        </div>
    </div>
  )
