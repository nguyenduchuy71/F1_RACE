...
function TeamInfo() {
    ...
    const team_name = location.pathname.split('/').slice(-1)[0].split('%20').join(' ');
    const [list_year_info, list_pos_info,
        list_background_color, getListTeamInfoResult] = useTeamInforStore((state) => [
  ...
  return (
    <div className="m-4 min-h-screen rounded-lg bg-white flex flex-col">
        ...
        <BarChartElement list_year_info={list_year_info} list_pos_info={list_pos_info} 
            ...
            y_label={"Years"} title={"Team's positioning over the years"}
        />;
    </div>
  )
}
